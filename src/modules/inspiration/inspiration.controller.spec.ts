...
describe('AppController', () => {
  ...
  beforeEach(async () => {
    const app: TestingModule = await Test.createTestingModule({
      ...
      providers: [{provide: getModelToken(Inspiration.name), useValue: inspirationModel},],
    }).compile();
  ...
  describe('POST /inspiration', () => {
    it('should return a 200 for a correctly created inspiration', () => {
      ...
      expect(appController.createInspiration(testInspo)).resolves.toHaveProperty("_id");
    });
    ...
    it('should return a 400 for an invalid type for the inspiration', async() => {
      ...
      const errors = await validate(invalid);
      expect(errors.length).not.toBe(0);
      expect(JSON.stringify(errors)).toContain("type must be one of the following values: responding, posting, synthesizing");
    });
  ...
  describe('PATCH /inspiration', () => {
    it('should return a 200 for a correctly updated inspiration', () => {
      ...
      expect(appController.updateInspiration('62b5f60a215d620007224dec', testInspo)).resolves.toHaveProperty("_id");
    });
