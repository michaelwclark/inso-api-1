...
export class MilestoneService {
    ...
    async createMilestoneForUser(
        ...
        await this.notificationService.createNotification(userId, userId, { header: `<h1 className="notification-header">You have achieved a badge!"</h1>`, text: `${milestoneName}`, type: 'badge'});
    ...
    async getMilestonesForUser(
        userId: Types.ObjectId
    ) {
        return await this.milestoneModel.find({ userId: userId });
    }

    async getMilestoneForUser(
        userId: Types.ObjectId,
        milestoneName: string
    ) {
        return await this.milestoneModel.findOne({ userId: userId, milestone: milestoneName });
    }
