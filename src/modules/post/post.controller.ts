  ...
  async createPost(
    ...
    if(discussion.facilitators.includes(new Types.ObjectId(req.user.userId))) {
      for await(const participant of discussion.participants) {
        await this.notificationService.createNotification(participant.user, { header: `<h1 className="notification-header"><span className="username">@${user.username}</span> responded in <a className="discussion-link" href="${process.env.DISCUSSION_REDIRECT}">${discussion.name}</a></h1>`, text: `${notificationText}`, type: 'post'});
      }
    }
    ...
    if(newPost.comment_for && newPost.userId !== req.userId) {
      await this.notificationService.createNotification(postForComment.userId, { header: `<h1 className="notification-header"><span className="username">@${user.username}</span> responded to your post sin <a className="discussion-link" href="${process.env.DISCUSSION_REDIRECT}">${discussion.name}</a></h1>`, text: `${notificationText}`, type: 'replies'})
    }
