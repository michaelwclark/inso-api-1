...
describe('AppController', () => {
  ...
  describe('POST /user/{userId}/calendar 200 STATUS', () => {
    it('Create Calendar Test', (done) => {
      appController
        .createCalendar('629a3aaa17d028a1f19f0e5c', validCalendar)
        .then((returnValue) => {
  ...
  describe('POST /user/{userId}/calendar 400 STATUS', () => {
    it('Test case invalid user', () => {
      const invalidUserError = new HttpException(
        'User id is not valid',
        HttpStatus.BAD_REQUEST,
      );
      return expect(
        appController.createCalendar('ThisIsNotAValidUserId', validCalendar),
      ).rejects.toThrow(invalidUserError);
    });

    //NO USERID, VALID CALENDAR WRITE DTO, SHOULD RETURN 400 STATUS
    it('Test case no user id', () => {
      const noUserError = new HttpException(
        'No user id provided',
        HttpStatus.BAD_REQUEST,
      );
      return expect(
        appController.createCalendar(null, validCalendar),
      ).rejects.toThrow(noUserError);
    });
