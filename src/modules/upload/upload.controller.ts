...
export class UploadController {
  ...
  @Post('upload')
  @ApiOperation({
    description: 'Uploads a file and returns the URL location of it',
  })
  @ApiQuery({
    name: 'type',
    required: true,
  ...
  @ApiForbiddenResponse({
    description:
      'The user is not allowed to upload a file for the entity id provided',
  })
  @ApiNotFoundResponse({
    description: 'The discussion, profile, or post was not found',
  })
  @ApiBadRequestResponse({
    description:
      'A file was not sent, the file is too large (>200MB), or the file does not have a valid extension.',
  })
  @ApiTags('File Upload')
  ...
  async uploadFile(
    ...
    if (file.size > 20971520) {
      throw new HttpException(
        'File is too large. File must be under 200MB',
    ...
    if (type === 'profile') {
      ...
      if (user._id.toString() !== request.user.userId) {
        throw new HttpException(
          `User ${id} is not the requester`,
          HttpStatus.FORBIDDEN,
        );
      }
      filePath =
        filePath + id + '/profile.' + file.originalname.split('.').pop();
    }
    // Discussion
    if (type === 'discussion') {
    ...
    if (type === 'post') {
      ...
      if (post.userId.toString() !== request.user.userId) {
        throw new HttpException(
          `Post ${id} does not belong to the user`,
          HttpStatus.FORBIDDEN,
        );
      }
      filePath = filePath + id + '/' + file.originalname;
