import { IsDefined, IsNotEmpty, IsNumber, IsString, Length, ValidateNested, Min, IsOptional } from 'class-validator';
import { ApiProperty } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import { HttpException, HttpStatus } from '@nestjs/common';

export class UpdateGradeCriteria { 
    @ApiProperty({
        name: 'criteria',
    ...
    constructor(partial: Partial<UpdateGradeCriteria>) {
        if(partial) {
...
export class UpdateGradeDTO {
    ...
    @IsNotEmpty()
    @ValidateNested()
    @Type(() => UpdateGradeCriteria)
    public criteria: UpdateGradeCriteria[];
    ...
    constructor(partial: Partial<UpdateGradeDTO>) {
        if(partial) {
            this.total = partial.total;
            this.criteria = partial.criteria.map(criteria => {
                criteria
                return new UpdateGradeCriteria(criteria)
            });
